...
<head>
    ...
    <title>{{ __('Tu entrada') }}</title>
</head>
<body style="margin:0; padding:0; background-color:#f4f4f5; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif; color:#18181b;">
    <div style="display:none; max-height:0; overflow:hidden; opacity:0;">
        {{ __('Tu entrada para :event', ['event' => $event->title]) }}
    </div>
        ...
        <div style="background-color:#ffffff; border-radius:12px; padding:32px 24px; border:1px solid #e4e4e7;">
            ...
            <p style="margin:0 0 24px; font-size:15px; line-height:1.6; color:#3f3f46;">
                ...
                @if ($event->venue)
                    {{ $event->venue }}<br>
                @endif
                <span style="color:#71717a;">{{ __('A nombre de :name', ['name' => $ticket->attendee_name]) }}</span>
            </p>
            ...
            <div style="text-align:center; padding:20px 0; background-color:#ffffff;">
                <img src="{{ $message->embedData($qrPng, 'nexo-events-ticket.png', 'image/png') }}"
                     alt="{{ __('Código QR de tu entrada') }}"
            ...
            <p style="margin:0; font-size:13px; line-height:1.6; color:#71717a;">
                {{ __('Si no ves el código, abrí tu entrada desde este enlace:') }}<br>
